...
class StrategyGuideRPS(private val moveValues: Map<String, Int> = mapOf("A" to 1, "B" to 2, "C" to 3)) {
    ...
    fun solve(lines: List<String>, operate : (String) -> (Int)) : Int {
        val summed : MutableList<Int> = mutableListOf()
        lines.forEach { summed.add(operate(it)) }
        return summed.sum()
    }

    val lambdaPartOne = { line : String -> // needs a describing name
    ...
    val lambdaPartTwo = { line : String -> // needs a describing name
...
fun main() {
    val lines = File("src/main/kotlin/advent_of_code_2022_day_2_RPS/input.txt").readLines()
    val guide = StrategyGuideRPS()

    println(guide.solve(lines, guide.lambdaPartOne))
    println(guide.solve(lines, guide.lambdaPartTwo))
}